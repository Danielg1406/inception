...
- `srcs/secrets/` contains the example secret files.
...
3. Create the local secret files:
...
4. Edit the secret files in `srcs/secrets/`.
...
You can also call Docker Compose directly if needed:
...
Stops the stack and removes the containers.
...
To verify the stack after changes, use:
...
```bash
bash -n srcs/requirements/mariadb/tools/setup.sh srcs/requirements/wordpress/tools/setup.sh
```

These checks confirm the compose file is syntactically valid and the startup scripts do not contain shell syntax errors.
